...
int main() {
	...
	int imageSize = sizeof(unsigned char)*imgHeight*imgWidth;
	unsigned char* greyData = nullptr, *blurredData = nullptr, *sobel_G_Data = nullptr, *sobel_theta_Data = nullptr, *Gx_Data = nullptr, *Gy_Data = nullptr;
	greyData = (unsigned char*)malloc(imageSize);
	blurredData = (unsigned char*)malloc(imageSize);
	sobel_G_Data = (unsigned char*)malloc(imageSize);
	sobel_theta_Data = (unsigned char*)malloc(imageSize);
	Gx_Data = (unsigned char*)malloc(imageSize);
	Gy_Data = (unsigned char*)malloc(imageSize);

	unsigned char* d_greyData = nullptr, *d_blurredData = nullptr, *d_sobel_G_Data = nullptr, *d_sobel_theta_Data = nullptr, *d_Gx = nullptr, *d_Gy = nullptr;;
	cudaMalloc(&d_greyData, imageSize);
	cudaMalloc(&d_blurredData, imageSize);
	cudaMalloc(&d_sobel_G_Data, imageSize);
	cudaMalloc(&d_sobel_theta_Data, imageSize);
	cudaMalloc(&d_Gx, imageSize);
	cudaMalloc(&d_Gy, imageSize);

	Mat source(inputImage.size(), CV_8U, createImageBuffer(imgWidth * imgHeight, &greyData));
	...
	Mat sobel_Gx(inputImage.size(), CV_8U, createImageBuffer(imgWidth * imgHeight, &Gx_Data));
	Mat sobel_Gy(inputImage.size(), CV_8U, createImageBuffer(imgWidth * imgHeight, &Gy_Data));
	...
	greyConvertor << <DimGrid, DimBlock >> > (d_input, d_greyData, imgWidth, imgHeight);
	cudaMemcpy(greyData, d_greyData, imageSize, cudaMemcpyDeviceToHost);
	convolve << <DimGrid, DimBlock >> > (d_greyData, d_gaussianFilter, imgWidth, imgHeight, 0, 0, 5, 5, d_blurredData);
	cudaMemcpy(blurredData, d_blurredData, imageSize, cudaMemcpyDeviceToHost);
	// sobel gradient convolutions (x&y padding is now 2 because there is a border of 2 around a 5x5 gaussian filtered image)
	convolve << <DimGrid, DimBlock >> > (d_blurredData, d_sobelFilter_Gx, imgWidth, imgHeight, 2, 2, 3, 3, d_Gx);
	cudaMemcpy(Gx_Data, d_Gx, imageSize, cudaMemcpyDeviceToHost);
	convolve << <DimGrid, DimBlock >> > (d_blurredData, d_sobelFilter_Gy, imgWidth, imgHeight, 2, 2, 3, 3, d_Gy);
	cudaMemcpy(Gy_Data, d_Gy, imageSize, cudaMemcpyDeviceToHost);
	pythagoras << <pBlocks, pThreads >> > (d_Gx, d_Gy, d_sobel_G_Data, d_sobel_theta_Data);
	cudaMemcpy(sobel_G_Data, d_sobel_G_Data, imageSize, cudaMemcpyDeviceToHost);
	cudaMemcpy(sobel_theta_Data, d_sobel_theta_Data, imageSize, cudaMemcpyDeviceToHost);
	...
	imshow("sobel_Gx_image", sobel_Gx);
	imshow("sobel_Gy_image", sobel_Gy);
	waitKey(0);
