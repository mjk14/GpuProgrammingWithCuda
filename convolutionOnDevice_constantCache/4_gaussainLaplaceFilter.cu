...
__global__ void Convolution_2D_globalMemory(unsigned char* imgInput, unsigned char* imgOutput, int height, int width, int channels) {
	...
	for (int c = 0; c < channels; c++)
	{
		...
		for (int i = 0; i < MASK_WIDTH; i++)
		{
			for (int j = 0; j < MASK_WIDTH; j++)
			{
				...
				{
					sum += imgInput[(filterRow * height + filterCol) * channels + c] * d_filter[i * MASK_WIDTH + j];
				}
				else { sum = 0; }
			}
		}
...
int main() {
	...
	Convolution_2D_globalMemory << <DimGrid, DimBlock >> > (d_input, d_output, imgHeight, imgWidth, imgChannels);
